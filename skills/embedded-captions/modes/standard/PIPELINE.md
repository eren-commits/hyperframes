...
3. **Two files, not one** (this is how the rail ends up _in front_ of the subject while the climax sits _behind_):
...
(`--ff` / `--cfill` / `--cacc`, climax fill/stroke), the named **FLOW*\*/CLIMAX*\* motion recipes** in `_motion.md`,
...
```jsonc
{
  ...
  "cfill": "#f4efe6",
  "cacc": "#caa14a", // fill + active-word accent (the template's tokens)
  ...
  "climax": {
    ...
    "top_pct": 36,
    "font_cqh": 34, // placement: centered, crossing the subject (safe-zones heroAnchor)
...
Worked example — _"She said she **LOVES** being treated like a girl."_ (LOVES = climax):
...
rail's page-flip**, exiting only when the sentence completes. The rail pages through the _non-peak_ words beneath
...
- **`index.html`**: climax `in` = the promoted word's spoken `start`; climax `out` = the **end of the thought**
  (after segment B's last word) — _not_ the promoted word's own end. So `HOLD` spans the page-flip, not just the word.
...
- **No `plan.json` in Standard mode** → the template-mode gates (`check-timing`, `check-occlusion`) don't run; `render-and-composite.sh` runs `check-overflow.cjs` on **both** `index.html` and `rail.html` (warning-only).
  ...
  (never shown in the rail — see _Rail ↔ climax hand-off_); climax holds to the end of its thought and exits at
...
- **One embed at a time**, spaced ≥ a beat apart. The promoted word is **handed off, never duplicated** in the
  rail (see _Rail ↔ climax hand-off_ above); if the embed and rail boxes would overlap _spatially_, the rail can
...
- **Rail lines must not overlap.** On continuous speech the next line's first word lands ~immediately after the
  previous line ends, so a line's exit has to _complete before_ the next begins (the skeleton pre-empts it at
