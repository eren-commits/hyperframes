...
function findSource(project) {
  for (const c of ["source.mp4"].concat(fs.readdirSync(project).filter((f) =>
    /\.(mp4|mov|webm|mkv|m4v|wav|m4a|mp3)$/i.test(f) && !/^(final|bg_plus_caps|fg_caps|rail|index)/.test(f)))) {
...
function main() {
  ...
  if (!process.argv[2]) { console.error("usage: audio-envelope.cjs <project-dir>"); process.exit(1); }
  const src = findSource(project);
  if (!src) { console.error("[envelope] no source media found"); process.exit(2); }

  // asetnsamples groups audio into fixed windows; astats prints RMS per window
  const sr = 16000, n = Math.round(sr * HOP);
  let out;
  try {
    ...
    out = cp.execFileSync("ffmpeg", [
      "-v", "info", "-i", src,
      "-map", "a:0",
      "-af", `aresample=${sr},aformat=channel_layouts=mono,asetnsamples=n=${n}:p=0,astats=metadata=1:reset=1,ametadata=mode=print:key=lavfi.astats.Overall.RMS_level:file=-`,
      "-f", "null", "-",
    ...
    out = (e.stdout || "") + "";
    if (!out.includes("RMS_level")) { console.error(`[envelope] ffmpeg failed: ${e.message}`); process.exit(2); }
  }
  const rmsDb = [];
  ...
  if (!rmsDb.length) { console.error("[envelope] no RMS windows parsed"); process.exit(2); }
  ...
  console.log(`[envelope] ${rms.length} windows @ ${HOP}s → envelope.json (loudest beat ~${peakAt.toFixed(2)}s)`);
}
main();
