...
async function main() {
  ...
  const num = (re, d) => { const m = html.match(re); return m ? parseFloat(m[1]) : d; };
  ...
  try {
    ...
    const waitTL = async () => {
      ...
      while (Date.now() - t0 < 12000) {
        if (await page.evaluate(() => !!(window.__timelines && window.__timelines.main))) return true;
    ...
    if (!hasTL) {                       // GSAP loads from CDN — a blip leaves no timeline; retry once
    ...
    if (!hasTL) {
      ...
      console.error("[overflow] ⚠ timeline did not register (GSAP CDN blocked?) — overflow check "
    ...
    for (const t of times) {
      ...
      for (const o of offenders) {
        const sides = Object.entries(o.off).filter(([, v]) => v > 0)
          .map(([s, v]) => `${s} ${v}px`).join(", ");
    ...
    if (found.size === 0) {
      ...
      console.error(`[overflow] ⚠ ${found.size} caption(s) leave the frame (custom mode — WARNING only, not blocking):`);
      for (const [text, info] of found)
        console.error(`           "${text}"  → off-frame: ${info.sides}  (@${info.t}s)`);
      console.error(`[overflow] if unintentional, reposition/resize; if it's deliberate bleed, ignore.`);
    }
  } finally {
    await browser.close();
  }
}
main().catch((e) => { console.error("[overflow]", e.message); process.exit(3); });
