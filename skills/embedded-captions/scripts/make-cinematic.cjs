...
function main() {
  ...
  // cinematic.json "drops": ["um", ...].
  {
    ...
    for (const b of blocks) for (const ln of (b.lines || [])) for (const w of ln._w) if (w.ti != null) consumed.add(w.ti);
    const dropOk = new Set((C.drops || []).map(norm));
    const FILLER = new Set(["um", "uh", "er", "ah", "hmm", "mm"]);
    const skipped = tr.map((w, i) => ({ w, i }))
      .filter(({ w, i }) => !consumed.has(i) && !dropOk.has(norm(w.text)) && !FILLER.has(norm(w.text)));
    if (skipped.length)
      die(`VERBATIM VIOLATION — ${skipped.length} transcript word(s) never authored (the matcher skipped them): ` +
  ...
  for (let i = 0; i < bw.length; i++) {
    ...
    if (i + 1 < bw.length)
      bw[i].out = Math.max(Math.min(bw[i].last, DUR - 0.05) + 0.15, Math.min(bw[i].out, bw[i + 1].first - 0.28));
  }
  ...
  blocks.forEach((b, bi) => {
    ...
    if (hs.length > 1) die(`block ${bi} has ${hs.length} hero lines — at most ONE hero per block (scarcity = per beat)`);
    if (hs.length === 1) allHeroBlocks.push({ b, bi, ln: hs[0], frac: fracOfLine(hs[0].css) || 0.24 });
  });
  ...
  const heroBlocks = allHeroBlocks.filter((h) => h.frac >= apexFracAll - 1e-6 && (h.ln.placement || "subject") !== "column");
  const minorHeroBis = new Set(allHeroBlocks.filter((h) => h.frac < apexFracAll - 1e-6 || (h.ln.placement || "subject") === "column").map((h) => h.bi));
  const columnApexBis = new Set(allHeroBlocks.filter((h) => h.frac >= apexFracAll - 1e-6 && (h.ln.placement || "subject") === "column").map((h) => h.bi));
  if (minorHeroBis.size) console.log(`[make-cinematic] ${minorHeroBis.size} minor hero(es) stay in the column flow (apex gets the lockup embed)`);
  const lockupName = (bi) => `lockup-b${bi}`;
  const lockupTemplate = C.planes && C.planes.lockup ? (typeof C.planes.lockup === "string" ? C.planes.lockup : C.planes.lockup.css) : null;
  ...
  for (const hRef of heroBlocks) {
    ...
    if (hb && hb.profile && hb.profile.length) {
      const ok = hb.profile.filter((b2) => b2.occPct >= 12 && b2.occPct <= 62 && (b2.bgLuma == null || b2.bgLuma <= 165));
      const pool = ok.length ? ok : hb.profile;
      const best = pool.reduce((x, y) => Math.abs(y.occPct - 40) < Math.abs(x.occPct - 40) ? y : x);
    ...
    const preH = (hRef.b.lines || []).filter((l, i) => l !== hRef.ln && i < heroIdx2)
      .reduce((s, l) => s + (fracOfLine(l.css) * 100 * 1.32 + Math.max(1.6, fracOfLine(l.css) * 100 * 0.38)), 0);
    const postH = (hRef.b.lines || []).filter((l, i) => l !== hRef.ln && i > heroIdx2)
      .reduce((s, l) => s + (fracOfLine(l.css) * 100 * 1.32 + Math.max(1.6, fracOfLine(l.css) * 100 * 0.38)), 0);
    ...
    const css = lockupTemplate
      ...
      : (a ? `top: ${top}%; left: ${a.xPct}%; width: ${a.wPct}%; text-align: center;`
  ...
  if (planes.hero) {
    if (heroBlocks.length) { planes[lockupName(heroBlocks[0].bi)] = planes.hero; }
    delete planes.hero;
  }
  ...
  if (sz && sz.zones) {
    ...
    for (const [pk, pv] of Object.entries(C.planes || {})) {
      ...
      const lm = css0.match(/left:\s*([\d.]+)%/), wm = css0.match(/width:\s*([\d.]+)%/);
      if (!lm || !wm) continue;
      const a0 = +lm[1], a1 = a0 + +wm[1];
  ...
  const lockupComp = bandLuma != null && bandLuma > 150
    ? " mix-blend-mode: normal; text-shadow: 0 1px 6px rgba(0,0,0,.65), 0 2px 16px rgba(0,0,0,.4);"
    : (bandLuma != null && bandLuma > 95 ? " text-shadow: 0 1px 5px rgba(0,0,0,.6), 0 2px 14px rgba(0,0,0,.35);" : "");
  ...
  // size sanity: narration below 0.05·h is whisper-tier (36px at 720p) — warn loudly
  {
    ...
    if (tiny.length) console.log(`[make-cinematic] ⚠ ${tiny.length} narration line(s) authored below 0.05·h (whisper tier) — body narration wants 0.055–0.07·h: ${tiny.slice(0, 3).map((l) => `"${l.ln.words.slice(0, 3).join(" ")}…"`).join(" ")}`);
  }
  ...
  for (const hRef of heroRefs) {
    ...
    if (ctx.length > 3) console.log(`[make-cinematic] ⚠ lockup-b${hRef.bi} has ${ctx.length} context lines — a lockup wants kicker + hero + tail; move narration to the column (a leading clause can be its own block)`);
    const moved = ctx.filter((l) => Math.abs(l.frac - ratioFrac) > 0.002).length;
    ctx.forEach((l) => {
      l.ln.css = (l.ln.css || "").replace(/font-size\s*:\s*[^;]+/, `font-size: calc(${ratioFrac} * var(--h))`);
      l.frac = ratioFrac;
    });
    if (moved) console.log(`[make-cinematic] lockup-b${hRef.bi}: context RATIO-LOCKED to 0.26×hero → ${ratioFrac}·h (${moved} line(s) re-sized)`);
  }
  const byPlane = {};
  ...
  for (const [pk, ls] of Object.entries(byPlane)) {
    ...
    if (hRef) {
      ...
      const estW = Math.min(W * 0.94, Math.max(fpx * 1.2, disp.replace(/\s/g, "").length * fpx * 0.6));
      const planeCss0 = (planes[pk] && planes[pk].css) || "";
      const pl = (planeCss0.match(/left:\s*([\d.]+)%/) || [0, 0])[1] * W / 100;
      ...
      orbit = { left: Math.round(heroLeft), right: Math.round(Math.max(0, pw - heroLeft - estW)), maxW: Math.round(Math.min(estW * 0.75, pw * 0.55)), side };
    }
    const pages = []; let page = [], slot = 0;
    for (const l of ls) {
      // the HERO occupies its slot between pre- and post-context, in spoken order
      if (hRef && hRef.slotPx == null && l.lockupPos === "post") { hRef.slotPx = Math.round(slot); slot += heroH; }
      ...
      if (page.length && !unbreakable && (slot + lineH > budget || (boundary && l.b.flip === true) || (boundary && pauseGap >= 0.6))) {
        pages.push(page); page = []; slot = 0;
      }
      l.slotPx = Math.round(slot); if (isLockup) l._orbit = orbit; slot += lineH; page.push(l);
    }
    if (hRef && hRef.slotPx == null) { hRef.slotPx = Math.round(slot); } // hero is the block's last line
    if (page.length) pages.push(page);
    pages.forEach((pg, pi) => {
      const nextFirst = pi + 1 < pages.length ? pages[pi + 1][0].first : null;
      const maxEnd = Math.max(...pg.map((l) => l.lastEnd));
      let out = nextFirst != null
      ...
      if (!isLockup) for (const hRef2 of heroRefs) {
        const lkIn = bw[hRef2.bi].first - 0.18;
        if (lkIn > maxEnd - 0.2 && lkIn < out) out = Math.max(maxEnd + 0.01, Math.min(out, lkIn - 0.28));
      }
      pg.forEach((l, li) => {
        l.pageIdx = pi;
        if (!isLockup) for (const hRef2 of heroRefs) {
          ...
          if (enter0 > lkIn2 && enter0 < lkOut) l._delayIn = +Math.min(l.first, Math.max(enter0, lkOut - 0.2)).toFixed(3);
        }
  ...
  for (const l of flat) {
    ...
    groups.push({
      id: l.gid, plane: l.plane, layer: globalFg ? "fg" : (l.ln.layer || l.b.layer || defLayer),
      ...(l.isMinorHero ? { hero: true, ...(columnApexBis.has(l.bi) ? {} : { minor: true }) } : {}),
      tone: l.isMinorHero ? (tones.hero || "present") : (l.ln.tone || tones.default || "soft"), allow_overlap: true,
      ...
      in: l.in, out: l.isMinorHero ? +Math.min(l.out, bw[l.bi].out + 0.3).toFixed(3) : l.out,
      ...
      words: l.ln._w.map((w, wi2) => ({
        ...
        start: w.start, end: Math.min(w.end, DUR - 0.05), ti: w.ti })),
    });
  }
  for (const hRef of heroRefs) {
    const { bi, ln } = hRef;
    const w0 = ln._w[0];
    groups.push({
      id: `h-${bi}`, hero: true, plane: hRef.plane, layer: ln.layer || (heroFeasible ? "bg" : "fg"),
      tone: tones.hero || "present", allow_overlap: true,
      in: +Math.max(0, w0.start - 0.02).toFixed(3), out: +bw[bi].out.toFixed(3),
      css: `position:absolute;left:0;right:0;top:${hRef.slotPx || 0}px; text-align:center; ` +
  ...
  for (let i = 1; i < heroGs.length; i++) {
    ...
    if (gap < 0.6) console.log(`[make-cinematic] ⚠ heroes "${heroGs[i - 1].words[0].text}" and "${heroGs[i].words[0].text}" are only ${gap.toFixed(2)}s apart — peaks need a beat of air between them (consider demoting one to an emphasis line)`);
  }
  ...
  const plan = {
    ...
    template: C.template || "cinematic-cream", compiled_by: "make-cinematic.cjs",
  ...
  // wrapped tail. Measure every page's REAL text heights in Chromium and re-stack.
  {
    ...
    for (const hRef of heroRefs) {
      ...
      if (!hg2) continue;
      (pagesByKey[key] = pagesByKey[key] || { t: +(hg2.out - 0.10).toFixed(2), lines: [] }).lines.push(
        { gid: hg2.id, slotPx: hRef.slotPx || 0, frac: fracOfLine(hg2.css) || 0.24, _hero: true });
    }
        ...
        for (const pg of Object.values(pagesByKey)) {
          ...
          for (const l of ordered) {
            ...
            if (Math.abs(top - l.slotPx) > 3) {
              ...
              if (l._hero) { const hr = heroRefs.find((h) => `h-${h.bi}` === l.gid); if (hr) hr.slotPx = Math.round(top); }
              l.slotPx = Math.round(top);
              changes++;
            }
            top += (realH || l.frac * H * 1.25) + Math.max(12, Math.round((l.frac || 0.05) * H * 0.38)); // box is lh1.12 — contains its own ink
          }
        }
        if (changes) {
  ...
  let dnaSizeCap = 0.34, dnaHeroCase = "none";
  try {
    const dnaLib2 = require("./lib-dna.cjs");
    const dn = plan.dna || dnaLib2.LEGACY[plan.template];
    if (dn) { const dj = dnaLib2.load(dn); if (dj.hero && dj.hero.sizeRange) dnaSizeCap = dj.hero.sizeRange[1]; if (dj.hero) dnaHeroCase = dj.hero.case || "none"; }
  } catch (e) {}
  ...
  const apexFrac = Math.max(...heroRefs.map((h) => fracOf((groups.find((g) => g.id === `h-${h.bi}`) || {}).css) || 0), 0);
  for (const hRef of heroRefs) {
    ...
    for (let pass = 0; pass < 5; pass++) {
      ...
      const others = caps.filter((c) => c.id !== heroG.id && !mateIds.has(c.id) && !/-glow$/.test(c.id || "") && c.cap_bbox && c.opacity > 0.05)
      ...
      if (f0 && hc.cap_bbox.w > 0) {
        if (hc.cap_bbox.w > maxW) {
          ...
          console.log(`[make-cinematic] hero "${heroG.words[0].text}" too WIDE ${Math.round(hc.cap_bbox.w)}px → ${f0}h→${nf.toFixed(3)}h`);
          heroG.css = setFrac(heroG.css, nf); changed = true;
        } else if (hc.cap_bbox.w >= maxW * 0.88) {
          if (pass === 0) console.log(`[make-cinematic] hero "${heroG.words[0].text}" at width CEILING (${Math.round(hc.cap_bbox.w)}/${Math.round(maxW)}px) — cannot enlarge.`);
          ...
          if (nf > f0 * 1.05 || fit > dnaSizeCap * 1.02) {
            if (nf > f0 * 1.001) console.log(`[make-cinematic] hero "${heroG.words[0].text}" TIMID (${Math.round(hc.cap_bbox.w)}px of ${Math.round(maxW)}px usable) → ${f0}h→${nf.toFixed(3)}h (width-fit max)`);
            ...
            if (fit > dnaSizeCap && wAtCap < maxW * 0.85 && capsy) {
              ...
              if (tr >= 0.04) {
                heroG.css = heroG.css.replace(/letter-spacing\s*:[^;]+;?\s*/g, "").replace(/text-indent\s*:[^;]+;?\s*/g, "")
                  + (heroG.css.trim().endsWith(";") || !heroG.css.trim() ? "" : ";")
                  + ` letter-spacing:${tr}em; text-indent:${tr}em;`;
                console.log(`[make-cinematic] hero "${disp}" short word at height cap (${Math.round(wAtCap)}px of ${Math.round(maxW)}px) → tracked +${tr}em to fill`);
              }
            }
      ...
      if (hits.length) {
        ...
        plan.planes[hRef.plane].css = planeCss.replace(/top:\s*[\d.]+%/, "top: " + newTop.toFixed(1) + "%");
        console.log(`[make-cinematic] hero "${heroG.words[0].text}" OVERLAPS ${hits.map((o) => o.id).join(",")} — lockup top ${curTop}% → ${newTop.toFixed(1)}%`);
        changed = true;
      }
      ...
      if (lockupMates.length && hc.cap_bbox && hc.cap_bbox.w > 0) {
        ...
        for (const m of lockupMates) {
          if (/left:\s*\d+px;right:auto/.test(m.css)) { const nc = m.css.replace(/left:\s*\d+px/, "left:" + relL + "px"); if (nc !== m.css) { m.css = nc; changed = true; } }
          else if (/right:\s*\d+px;left:auto/.test(m.css)) { const nc = m.css.replace(/right:\s*\d+px/, "right:" + relR + "px"); if (nc !== m.css) { m.css = nc; changed = true; } }
        }
      }
      ...
      if (lockupMates.length) {
        ...
        for (const e of entries) {
          if (Math.abs(top - e.curTop) > 3) { e.g.css = e.g.css.replace(/top:\s*-?[\d.]+px/, "top:" + Math.round(top) + "px"); changed = true; }
        ...
        if (curTopPct > maxTopPct + 0.3) {
          plan.planes[hRef.plane].css = planeCss3.replace(/top:\s*[\d.]+%/, "top: " + maxTopPct.toFixed(1) + "%");
