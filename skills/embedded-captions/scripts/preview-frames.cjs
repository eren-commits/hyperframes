...
const HF_ROOTS = [process.env.HYPERFRAMES_ROOT, path.resolve(__dirname, "../../.."), path.join(os.homedir(), "Downloads", "hyperframes")].filter(Boolean);
  ...
  try {
    ...
    if (gsapSource) {
      ...
      page.on("request", (req) => {
        ...
        else if (req.resourceType() === "media") req.abort(); // a-roll pixels come from frames_bg
        else req.continue();
      });
    }
...
async function main() {
  ...
  if (!process.argv[2]) { console.error("usage: preview-frames.cjs <project-dir> [times...]"); process.exit(1); }
  ...
  const exe = process.platform === "darwin" ? "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" : "/usr/bin/google-chrome";
  const browser = await puppeteer.launch({ headless: "new", executablePath: fs.existsSync(exe) ? exe : undefined,
  ...
  try {
    for (const t of times) {
      ...
      if (!fs.existsSync(bg)) { console.error(`[preview] no bg frame for t=${t}`); continue; }
    ...
    if (outs.length) {
      ...
      for (let i = 0; i < outs.length; i++)
        comps.push({ input: await sharp(outs[i].out).resize(TW, TH).toBuffer(), left: (i % 4) * TW, top: Math.floor(i / 4) * TH });
      const rows = Math.ceil(outs.length / 4);
      await sharp({ create: { width: 4 * TW, height: rows * TH, channels: 3, background: { r: 16, g: 16, b: 16 } } })
        .composite(comps).png().toFile(path.join(outDir, "sheet.png"));
      console.log(`[preview] contact sheet → ${path.join(outDir, "sheet.png")}  (${outs.length} frames @ ${times.join(", ")}s)`);
    }
  } finally {
    await Promise.race([browser.close().catch(() => {}), new Promise((r) => setTimeout(r, 8000))]);
  }
}
main().then(() => process.exit(0)).catch((e) => { console.error("[preview]", e.message); process.exit(1); });
